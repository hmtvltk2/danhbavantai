				...
				if ( ! empty( $data_value['post_title'] ) ) {
					...
					if ( is_object( $page ) && $page->ID ) {
						...
						if ( ! empty( $panels_data['widgets'] ) ) {
							foreach ( $panels_data['widgets'] as $instance_id => $widget_instance ) {
								...
								} elseif ( ! empty( $data_value['data_update'] ) ) {
									$instance_class = $widget_instance['panels_info']['class'];

									foreach ( $data_value['data_update'] as $dropdown_type => $dropdown_data ) {
										if ( ! in_array( $dropdown_type, array( 'dropdown_pages', 'dropdown_categories' ) ) ) {
											continue;
										}
